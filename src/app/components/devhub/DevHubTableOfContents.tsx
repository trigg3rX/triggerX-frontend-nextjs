...
export const DevHubTableOfContents: React.FC<DevHubTableOfContentsProps> = ({
  ...
  const handleHeadingClick = (e: React.MouseEvent<HTMLAnchorElement>, h2Heading: string) => {
    e.preventDefault();
  ...
  const renderHeadingList = (isMobile = false) => (
    <ul className={`space-y-2 font-actay ${isMobile ? 'absolute w-full bg-[#141313] text-white rounded-lg border border-[#5F5F5F] mt-2 shadow-lg z-10' : 'hidden md:block'}`}>
      {headingPairs.map((pair, index) => (
        <li key={index} className={`py-2 px-2 ${isMobile ? '' : 'text-xs lg:text-sm 2xl:text-base'}`}>
          <a
            href={`#${pair.h2Heading}`}
  ...
  return (
    <aside className={`w-full md:w-1/4 min-w-[180px] lg:min-w-[230px] md:sticky top-24 h-full ${className}`}>
      {/* Mobile Dropdown */}
