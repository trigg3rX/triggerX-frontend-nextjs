...
export async function PUT(
  ...
  try {
    ...
    devLog(`[API Route] [${traceId}] Proxying PUT request to /api/jobs/delete/${jobId}`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data || "Failed to delete job" },
    ...
    devLog("[API Route] Error in PUT /api/jobs/delete:", error);
    return NextResponse.json(
