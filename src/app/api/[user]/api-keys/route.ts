...
export async function POST(
  ...
  try {
    ...
    devLog(`[API Route] [${traceId}] Proxying POST request to /api/${user}/api-keys`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data || "Failed to create API key" },
    ...
    devLog("[API Route] Error in POST /api/[user]/api-keys:", error);
    return NextResponse.json(
