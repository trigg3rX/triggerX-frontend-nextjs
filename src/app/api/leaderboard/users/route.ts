...
export async function GET(req: NextRequest) {
  try {
    ...
    devLog(`[API Route] [${traceId}] Proxying GET request to /api/leaderboard/users`);

    const response = await fetch(`${API_BASE_URL}/api/leaderboard/users`, {
      method: "GET",
      headers: {
    ...
    devLog("[API Route] Error in GET /api/leaderboard/users:", error);
    return NextResponse.json(
